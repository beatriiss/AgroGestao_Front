...
export const getPropertyCultivations = async (propertyId) => {
  try {
    const response = await axios.get(`${url}/cultivations/property/${propertyId}`);
    return response.data.cultivations; // Retorna os dados da resposta da API
  } catch (error) {
